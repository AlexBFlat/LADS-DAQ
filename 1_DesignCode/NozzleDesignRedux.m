...
Keth = 2.57e3;   % Thermal conductivity of ethanol in J/kgK.
...
[F1,F2,F3,F4,F5,F6,P0,P1, P2, P3, P4, P5, P6, F1v, F2v, F3v, F4v, F5v, F6v,R1] = RaoNozzleGeomM(Dc, Dt, De, Lcyl, R2rat, 40, epsilon, Lengthfrac);
...
muc(i) = 1.2e-3; % Ethanol viscosity at bulk temp.
mucw(i) = 1.2e-3; % Ethanol viscosity at coolant sidewall temp.
...
dx = abs(Ax(i+1) - Ax(i));
dy = abs(Dx(i+1) - Dx(i));
SAch(i) = 2*pi*Dx(i)*sqrt(dx^2+dy^2)*thetach(i)/360;
