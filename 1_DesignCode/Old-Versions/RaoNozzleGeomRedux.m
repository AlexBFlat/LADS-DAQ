...
% Constants to set
function [F1,F2,F3,F4,F5,F6, P0,P1, P2, P3, P4, P5, P6, F1v, F2v, F3v, F4v, F5v, F6v,R1] = RaoNozzleGeomRedux(Dc, Dt, De,R2rat, b, epsilon,epsilonc,Lengthfrac,Lstar)
...
At = 4*Rt^2;       % Solves for throat area in m2.
Ac = 4*Rc^2;       % Solves for chamber area in m2.
Ae = 4*Re^2;       % solves for exit area in m2.
...
func = At*(Lcs*epsilonc+1/3*sqrt(At/pi)*cotd(b)*(epsilonc^(1/3)-1))-Vc; % Solves for chamber barrel length in m.
