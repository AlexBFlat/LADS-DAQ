...
injstf = 4;    % Defines stiffness of injctor.
Pcns = 88252.89;% Chamber pressure in psia.
P1eth = Pcns*injstf;
dPeth = P1eth - Pcns;
P1ox = Pcns*injstf;
dPox = 500*6894.76;
...
Cpf = 4184;
rhof = 1000;
muox = 1.002e-3; % Viscosity of liquid oxygen in Pas.
rhoox = 1000; % oxygen density kg/m3.
nuox = muox/rhoox;
nueth = muf/rhof;
mdotox = mdot*of/(of+1);
mdotf = mdot/(of+1);
sigmaw = 0.8e-3; % nozzle wall thickness in m.
vox = 101325; % Vapor pressure of oxygen in Pa.
...
%[A1, mu1] = Afind(a1,lbn1,1); % Finds geometric characteristic stage 1 A1.
%phi1 = phicalc(A1,mu1);
%Rn1 = .475*sqrt(mdotox/(mu1*sqrt(rhoox*dPox)));
%Rin1 = Rbin*Rn1;
%rin1 = sqrt((Rin1*Rn1)/(n1*A1));
%Rein1 = 2*mdotox/(pi*sqrt(n1)*rin1*rhoox*nuox);
%lin1 = 3*rin1;
%ln1 = 2*Rn1;
%ls1 = 3*Rn1;
%R1 = Rn1+sigmaw;
